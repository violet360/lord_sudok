...
def display(in_img, rects, colour=255):
...
def extract_digit(img, rect, size):
	...
	digit = slash_rect(img, rect)

	h, w = digit.shape[:2]
	margin = int(np.mean([h, w]) / 2.5)
	_, bbox, seed = find_largest_feature(digit, [margin, margin], [w - margin, h - margin])
	digit = slash_rect(digit, bbox)
...
cropped = crop_and_warp(path, corners)
squares = infer_grid(cropped)
x  = display(cropped, squares)
show_image(x)